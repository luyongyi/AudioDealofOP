...
如果仅需要标准音源分析，普通响度计权处理，普通THD处理，可以直接移步此项目， https://github.com/endolith/waveform-analysis/tree/master  
__如果需要非标准音乐音频杂音识别思路，可以继续往下看__
...
```
## 其他类型杂音 
这里是处理其他类型杂音的思路，会抽取几个典型的来讲  

### 破音
#### 内链路破音
内链路就是整个音频处理未经过实际空气传播部分，仅仅在线路内做模拟/数字处理的链路过程，包括宽带传输等。内链路破音处理方法要稍微简单一些，可以看到基本上破音都基本有方波特征，如图所示
<div align="center">
<figure>
    <img src="photo/破音特征，用红色箭头标记.png" alt="图片描述" />
    <figcaption>破音特征图:部分用红色箭头展示</figcaption>
</figure>
</div>  

这里有两个思路  
1.当有帧波形值持续等于0dBFS，即判断为破音  
2.遍历每个采样点值，当前值与前采样点做diff，持续为0的时间段是破音  
两个思路都会有缺点，1的缺点是会漏掉因为增益控制而压下来的破音，很多音频最大dBFS不会碰到0dBFS刻度线。2缺点是如果有部分音乐本身就是有静音段，那么diff值肯定也是0，但是静音段不是破音，会造成误判，所以2也是有问题的。  
解决办法是再审查这里破音的特征，首先破音采样值肯定是会顶到当前文件的最大采样点值，但是又不一定是16bit最大值0dB。所以我们只需要重新计算文件BFS值即可，更改log内被除数为当前未见最大的采样值，那么文件内最大值便是0dB。可以直接使用1做判断，不会出错。
```python
#代码暂定
```
#### 外链路破音
如果实际音频已经经过空气传播再次采集，那么特征将会完全不一样，整个波形特征不会有如此平直的0dB直线，而是呈现跟正常音频相似的抖动。  
这里讨论的是外放形式的破音，所以还是有特征可以抓取，但是要对音源做标准化限制，以下是思路：  
1.抓频域泄漏特征，在高频设定频率域值，但是要对应歌曲源音源配合不能有超过该频段的声音    
<div align="center">
<figure>
    <img src="photo/破音的频率泄漏特征（用蓝框标记）.png" alt="图片描述" />
    <figcaption>破音特征图:频率泄漏特征</figcaption>
</figure>
</div> 

2.偶现破音还可以看前后相关性，播放两端相同音源，抓相关性可以看出不相关场景，再通过频谱和波形二次判断抓出破音异常。  
...
### 响度
同外放处理，通话响度可以参考K计权，使用A计权也可以，参考顶部第三方库链接即可
